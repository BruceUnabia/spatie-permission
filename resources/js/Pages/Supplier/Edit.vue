<script setup>
    ...
    const form = useForm({
        ...
        contact_no: props.suppliers.contact_no,
    ...
    const submit = () =>{
        form.put('/suppliers/' + props.suppliers.id)
        form.name = ""
        form.contact_no = ""
        form.email = ""
        form.address = ""
...
<template>
     <Head title="Create Supplier" />
      <Sidebar>
        <template #header>
          <div class="flex">
                <h2 class="flex-1 font-semibold text-xl text-gray-800 leading-tight">Update Supplier | {{ suppliers.name }}</h2>
